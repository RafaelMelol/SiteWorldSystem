...
export function Header() {
  ...
  // Fecha os menus ao trocar de rota. Ajustar o state durante a render,
  // em vez de num effect, evita uma renderização extra após a navegação.
                ...
                {item.children ? (
                  <>
                    <button
                      ...
                      className="flex items-center gap-1 rounded-lg px-4 py-2 text-sm font-medium text-foreground/80 transition-colors hover:bg-surface-muted hover:text-foreground"
                    >
                      {item.label}
                      <ChevronDown
                        aria-hidden
                  ...
                  <Link
                    href={item.href!}
                    className="block rounded-lg px-4 py-2 text-sm font-medium text-foreground/80 transition-colors hover:bg-surface-muted hover:text-foreground"
                  >
                    {item.label}
                  </Link>
                )}
...
function MobileDropdown({ item }: { item: (typeof mainNav)[number] }) {
